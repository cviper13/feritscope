...
impl RadarRenderer {
    ...
    fn render_target(
        ...
        let pos = projection.studs_to_screen(tracked.info.position.x, tracked.info.position.y);
        ...
        painter.add(egui::Shape::closed_line(points, Stroke::new(display.target_stroke, color)));

        // Draw heading indicator
        let heading_rad = (tracked.info.heading - 90.0).to_radians(); // -90 to align with North
        let heading_len = size * 2.0;
        let heading_end =
            pos + Vec2::new(heading_rad.cos() * heading_len, heading_rad.sin() * heading_len);

        painter.line_segment([pos, heading_end], Stroke::new(display.target_stroke * 0.7, color));
    }
    ...
    fn render_vector(
        ...
        let distance_studs = (gs_knots *
            (studs_per_knot_per_sec as f64) *
            (seconds_ahead as f64)) as f64;
    ...
    fn format_data_tag(&self, tracked: &TrackedAircraft, display: &DisplayConfig) -> Vec<String> {
        ...
        vec![tracked.callsign.clone(), format!("F{:03} {:03}KT", altitude, gs)]
    }
